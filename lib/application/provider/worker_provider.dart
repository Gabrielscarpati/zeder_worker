...
class WorkerProvider with ChangeNotifier {
  ...
  Future<String> updateUserImage({required BuildContext context}) async {
    ...
    String profilePicture = await SaveImage(image: await convertFileToUint8List(photoFile!), path: 'userUrl', id: await FirebaseManager().getUserCurrentID()).saveAndGetUrl();
    await WorkerController().atualizarWorkerPicture(await WorkerController().buscarWorker(firebaseController.getCurrentUser()!.uid),profilePicture);
    photoFile = null;
    ShowSnackBar(context: context).showErrorSnackBar(message: 'Imagem alterada com sucesso.', color: DSColors.secondary);
